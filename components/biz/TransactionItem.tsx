...
export default function TransactionItem({
    ...
    const { activeAccount } = useDataStore()
    return (
        <View className="flex-row justify-between items-center py-6 px-5 bg-white bg-gray-200 dark:bg-charcoal-800">
            <View className="flex-1 flex-row items-center">
                ...
                <View className="flex-1">
                    <Text className="text-base font-medium text-gray-900 mb-1 dark:text-gray-100">
                        {title}
                    </Text>
            ...
            <Text
                className={`text-xl font-bold ${type === 'income' ? 'text-green-500' : 'text-red-500'}`}
            >
                {type === 'income' ? '+' : '-'}
                {CURRENCIES[activeAccount!.currency as keyof typeof CURRENCIES]?.char || '￥'}
