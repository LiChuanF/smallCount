        ...
        `}
          ...
          `}
                    ...
                    <View className="flex-1">
                        {/* 账户名称 */}
                        <Text className={`text-base text-text ${textColorClass}`}>{item.name}</Text>
                        {/* 账户类型和余额 */}
                        <View className="flex-row justify-between items-center mt-1">
                            <Text className="text-sm font-medium text-gray-700 dark:text-gray-300">
                                总资产：
                                {(item?.currency &&
                                    CURRENCIES[item.currency as keyof typeof CURRENCIES]?.char) ||
                                    '￥'}
                                {item.balance || '0.00'}
                            </Text>
                        </View>
    ...
    return (
        ...
        >
            ...
            <View className="flex-1 justify-end bg-black/50">
                ...
                {/* 内容区域 */}
                <View
                    className="bg-card dark:bg-card-dark w-full rounded-t-3xl shadow-2xl h-[65%]"
                    style={{
                ...
                >
                    ...
                    {data.length > 0 ? (
                        ...
                        <View className="flex-1 justify-center items-center">
                            ...
                            <Text className="text-lg text-text mt-4">暂无账户</Text>
