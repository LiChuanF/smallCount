...
export class AccountRepository extends BaseRepository<Account> {
    ...
    async setAsDefault(userId: string, accountId: string): Promise<void> {
        await this.db.transaction(async tx => {
            ...
            // 2. 将目标账户设为默认
            await tx.update(accounts).set({ isDefault: true }).where(eq(accounts.id, accountId))
        })
    }

    // 设置账户为已归档
    async setAsArchived(accountId: string): Promise<void> {
        await this.db.update(accounts).set({ isArchived: true }).where(eq(accounts.id, accountId))
    }
