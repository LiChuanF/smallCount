...
export const BudgetService = {
    ...
    async createBudget(budgetData: Omit<NewBudget, 'id' | 'createdAt' | 'updatedAt'>) {
        ...
        if (
            !budgetData.period ||
            !['daily', 'weekly', 'monthly', 'yearly'].includes(budgetData.period)
        ) {
        ...
        if (
            ...
            (!budgetData.month || budgetData.month < 1 || budgetData.month > 12)
        ) {
        ...
        if (
            ...
            (!budgetData.week || budgetData.week < 1 || budgetData.week > 53)
        ) {
     ...
     * @returns 预算使用情况
     */
    async getBudgetUsage(accountId: string, budgetId: string, startDate: Date, endDate: Date) {
        if (!accountId || accountId.trim().length === 0) {
            throw new Error('账户ID不能为空')
        }
    ...
    async getMonthlyBudgetOverview(accountId: string, year: number, month: number) {
        ...
        const averagePercentage =
            budgetsWithUsage.length > 0
                ? budgetsWithUsage.reduce((sum, item) => sum + item.percentage, 0) /
                  budgetsWithUsage.length
                : 0
     ...
     * @returns 是否超支
     */
    async isBudgetOverLimit(accountId: string, budgetId: string, startDate: Date, endDate: Date) {
