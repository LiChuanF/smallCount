...
async function ensureDefaultAccount(userId: string) {
  ...
  if (list.length === 0) {
    ...
    return { count: 1, defaultAccountId: acc.id };
  }
  
  const def = list.find((a) => a.isDefault === true);
  if (!def) {
    await AccountService.setAsDefault(userId, list[0].id);
    return { count: list.length, defaultAccountId: list[0].id };
  }
  
  return { count: list.length, defaultAccountId: def.id };
}
...
export const SeedService = {
  ...
  async initDefaultData() {
    ...
    const existing = await new UserRepository().findByUsername(DEFAULT_USERNAME);
    if (!existing) {
      throw new Error('默认用户不存在');
    } else {
      console.log(`默认用户存在: ${existing.displayName} (ID: ${existing.id})`);
    }


    // 3. 创建一些交易数据 - 如果没有交易数据 模拟用户进行几笔交易
    const tradeRepo = new TransactionRepository();
    console.log(`默认账户ID: ${account.defaultAccountId} (ID: ${existing.id})`);
