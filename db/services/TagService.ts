...
export const TagService = {
    ...
    async createTag(tagData: Omit<NewTag, 'id' | 'updatedAt' | 'createdAt'>) {
        ...
        const existingTag = existingTags.find(
            tag =>
                tag.name.toLowerCase() === tagData.name.toLowerCase() && tag.type === tagData.type,
        )
     ...
     * @returns 更新后的标签
     */
    async updateTag(tagId: string, updateData: Partial<Omit<NewTag, 'id' | 'createdAt'>>) {
        // 获取当前标签信息
        ...
        if (updateData.name) {
            const allTags = await tagRepo.findAll()
            const existingTagWithSameName = allTags.find(
                tag =>
                    tag.id !== tagId &&
            ...
            if (existingTagWithSameName) {
                throw new Error(
                    `已存在相同名称的${(updateData.type || existingTag.type) === 'expense' ? '支出' : '收入'}标签`,
                )
            }
        }
    ...
    async createTagsBatch(tagsData: Omit<NewTag, 'id' | 'createdAt'>[]) {
        ...
        const existingTagNames = new Set(
            existingTags.map(tag => `${tag.name.toLowerCase()}_${tag.type}`),
        )
        ...
        for (const tagData of tagsData) {
            ...
            if (existingTagNames.has(key)) {
                throw new Error(
                    `已存在相同名称的${tagData.type === 'expense' ? '支出' : '收入'}标签: ${tagData.name}`,
                )
            }
        }
