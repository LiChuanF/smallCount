...
export const TransactionService = {
    ...
    async enrichTransactionsWithTagsAndPaymentMethods(
        ...
        return result.items.map(tx => ({
            ...
            paymentMethod: tx.paymentMethodId
                ? paymentMethodsMap.get(tx.paymentMethodId)
                : undefined,
        }))
    },
    ...
    async getTransactionsByMonth(
        ...
        // 使用封装的方法添加标签和支付方式信息
        const transactionsWithTagsAndPaymentMethods =
            await this.enrichTransactionsWithTagsAndPaymentMethods(result)
     ...
     * @returns 交易记录列表
     */
    async getTransactionsByYear(accountId: string, year: number) {
        // 参数校验
        if (year < 2000 || year > 2100) {
            throw new Error('年份必须在2000-2100之间')
        }
    ...
    async getMonthlyStats(accountId: string, year: number, month: number) {
        ...
        return {
            ...
            averageTransactionAmount:
                transactions.length > 0 ? (totalIncome + totalExpense) / transactions.length : 0,
        }
    },
    ...
    ): Promise<TransactionWithTagAndPaymentMethod | undefined> {
    ...
    async getComparisonData(
        ...
        // 获取对比时期的金额
        const compareAmount = await this.getTotalAmountByPeriod(
            accountId,
            compareYear,
            compareMonth,
            type,
    ...
    async createSimpleTransaction(transactionData: {
        ...
        const description =
            ...
            `${transactionData.type === 'income' ? '收入' : '支出'}交易`
